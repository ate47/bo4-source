...
// Namespace blackboard/namespace_d5387423
...
function private bb_getlocomotionmovementtype() {
    if (!ai::getaiattribute(self, "disablesprint")) {
        ...
        if (!isdefined(self.var_472ef77a)) {
            self.var_472ef77a = 0;
        }
        now = gettime();
        if (now >= self.var_472ef77a) {
            self.var_bdd3f1c1 = getactorteamcountradius(self.origin, 120, self.team, #"neutral");
            self.var_472ef77a = now + 500;
        }
        if (self.var_bdd3f1c1 >= 3) {
