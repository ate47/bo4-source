...
function get_best_brutus_spawn_pos(zone_name) {
    ...
    for (i = 0; i < level.var_57d61da9.size; i++) {
        if (isdefined(zone_name) && level.var_57d61da9[i].zone_name != zone_name) {
            ...
            jumpiffalse(a_players_in_zone.size) LOC_000000ba;
        } else {
        LOC_000000ba:
    ...
    if (isdefined(var_b9ed2d9f)) {
        return level.var_57d61da9[var_b9ed2d9f];
    } else {
        return undefined;
    }
}
...
function get_brutus_spawn_pos_val(var_ed72e56b) {
    ...
    for (i = 0; i < a_players_in_zone.size; i++) {
        if (self findpath(var_ed72e56b.origin, a_players_in_zone[i].origin, 0, 0)) {
            ...
            var_6404f532 = var_6404f532 + math::linear_map(n_dist, 128, 4016, 0, level.var_549a1590);
        }
    }
    if (var_6404f532 > level.var_549a1590) {
        var_6404f532 = level.var_549a1590;
    }
    n_score = n_score + var_6404f532;
    return n_score;
}
...
function brutus_round_tracker() {
    ...
    var_1d4f2f0b = level.round_spawn_func;
    var_9d2417de = level.round_wait_func;
    while (1) {
        ...
        if (level.round_number < 6 && isdefined(level.is_forever_solo_game) && level.is_forever_solo_game && !(isdefined(level.var_cab8d080) && level.var_cab8d080)) {
            ...
            continue;
        } else if (level.var_33be9958 <= level.round_number) {
...
function brutus_death() {
    ...
    if (isplayer(self.attacker)) {
        ...
        foreach (player in a_players) {
            ...
            player zm_score::add_to_player_score(var_85fd81ba);
...
function wait_on_box_alarm() {
    while (1) {
        ...
        if (level.var_a33d6e46 >= level.var_87dac1c6) {
            ...
            } else if (rand <= level.var_cc879537) {
            ...
            } else if (level.var_cc879537 < level.var_49efbc6d) {
