        ...
        if (isdefined(target)) {
            ...
            for (i = 0; i < players.size; i++) {
                ...
                if (isdefined(team)) {
                    ...
                    jumpiffalse(strstartswith(team, "<unknown string>") && target.team != players[i].team) LOC_0000021c;
                } else {
                LOC_0000021c:
                    goal = undefined;
                ...
                if (level.gametype == "<unknown string>" && isdefined(level.radios)) {
                    if (!level.devgui_show_hq) {
                        for (i = 0; i < level.radios.size; i++) {
                            color = (1, 0, 0);
                            level dev_spawn::showonespawnpoint(level.radios[i], color, "<unknown string>", 32, "<unknown string>");
                        }
                    } else {
