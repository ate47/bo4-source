...
function private __init__() {
    ...
    level.var_9cddbf4e = [];
    level.var_9cddbf4e[#"p8_fxanim_wz_supply_stash_01_mod"] = {#var_b9492c6:#"hash_32f9ba3b1da75ed5", #open_sound:#"hash_3462cfb200a2367"};
    level.var_9cddbf4e[#"p8_fxanim_wz_supply_stash_04_mod"] = {#var_b9492c6:#"hash_32f9ba3b1da75ed5", #open_sound:#"hash_3462cfb200a2367"};
    level.var_9cddbf4e[#"p8_fxanim_wz_death_stash_mod"] = {#var_b9492c6:#"hash_70fb2ee1b706a28a", #open_sound:#"hash_5e8b0f6cade25ff6"};
    level.var_9cddbf4e[#"hash_1dcbe8021fb16344"] = {#var_b9492c6:#"hash_6fcb29cae6678d93", #open_sound:#"hash_56b5b65c141f4629"};
    level.var_9cddbf4e[#"p8_fxanim_wz_supply_stash_ammo_mod"] = {#var_b9492c6:#"hash_3e62bcbd6460ff44", #open_sound:#"hash_f743d336f8b7764"};
    level.var_9cddbf4e[#"hash_574076754776e003"] = {#var_b9492c6:#"hash_22f426a8593609e8", #open_sound:#"hash_36e23ce3e5f7e4c0"};
    level.var_9cddbf4e[#"wpn_t7_drop_box_wz"] = {#var_b9492c6:#"hash_2b751d50426093db", #open_sound:#"hash_613f8a1669f8b231"};
...
function private function_d045e83b(item, player, networkid, itemid, itemcount, var_aec6fa7f, slot) {
    ...
    if (!self item_inventory::can_pickup_ammo(item)) {
        return (isdefined(item.var_a6762160.amount) ? item.var_a6762160.amount : isdefined(var_aec6fa7f) ? var_aec6fa7f : 1);
    }
...
function private function_199c092d(supplystash, player = undefined) {
    ...
    foreach (item in var_76c7cb8a) {
        ...
        if (item_world_util::can_pick_up(item) && !isdefined(var_cf23afee[item.var_a6762160.weapon.name])) {
            consume_item(item);
        } else if (isdefined(player.var_fbcc86d3) && isdefined(player.var_fbcc86d3[item.var_a6762160.weapon.name])) {
            consume_item(item);
        } else if (isdefined(var_cf23afee[item.var_a6762160.weapon.name])) {
            function_54ca5536(item.id, -1);
            resetitem = level flagsys::get(#"item_world_reset");
            function_bfc28859(3, item.id, resetitem);
            continue;
...
function function_de2018e3(item, player, slotid = undefined) {
    ...
    /#
        assertmsg("<unknown string>" + var_a6762160.name + "<unknown string>");
    #/
}
...
function pickup_item(item, var_22be503 = 1, var_26a492bc = 0) {
    ...
    if (isdefined(var_fa3df96)) {
        ...
        if (var_77e61fc6 == 0) {
            ...
            stashitem = item.hidetime === -1;
            stashitem = stashitem & ~(isdefined(item.deathstash) ? item.deathstash : 0);
            dropitem = self item_drop::drop_item(item.var_a6762160.weapon, var_77e61fc6, item.amount, item.id, item.origin, item.angles, stashitem, undefined, isdefined(item.targetnamehash) ? item.targetnamehash : item.targetname, undefined, undefined, 0);
...
function function_83ddce0f(item, inventoryslot) {
    ...
    if (var_77e61fc6 == 0) {
        ...
        dropitem = self item_drop::drop_item(item.var_a6762160.weapon, item.count, item.amount, item.id, item.origin, item.angles, 0, undefined, isdefined(item.targetnamehash) ? item.targetnamehash : item.targetname, undefined, undefined, 0);
        if (isdefined(dropitem)) {
            dropitem.origin = item.origin;
            dropitem.angles = item.angles;
            consume_item(item);
